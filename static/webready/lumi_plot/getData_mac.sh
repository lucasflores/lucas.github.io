...
CHECK_DIFF_PAGE="$(diff <(tail -1 lumi_plot/pages.csv | awk '{print $2}') <(echo "`mdls thesis.pdf | grep Pages | awk '{print $3}'`"))"
...
PAGE_COMPILE="$(echo "`mdls thesis.pdf | grep Pages | awk '{print $3}'`")"
PAGE_HISTORY="$(tail -1 lumi_plot/pages.csv | awk '{print $2}')"
# echo PAGE_COMPILE $PAGE_COMPILE
# echo PAGE_HISTORY $PAGE_HISTORY
...
then 
  ...
  echo "`date '+%d/%m/%y,'` `mdls thesis.pdf | grep Pages | awk '{print $3}'`" >> lumi_plot/pages.csv 
fi 
